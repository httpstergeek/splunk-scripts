...
class SPLK():
    ...
    def requestsession(self):
        ...
        print '{0}{1}'.format(self.url, '/services/auth/login')
    ...
    def createjob(self, search_query):
        ...
        :return:
        """
        data = dict(search=search_query, output_mode=self.output_mode)
    ...
    def getresults(self, job_id, outputmode):
        ...
        request = urllib2.Request('{0}{1}{2}{3}'.format(self.url, search_url, job_id, '/results?output_mode={0}'.format(outputmode)),
                                  headers=self.auth_header)
        connection = urllib2.urlopen(request)
...
if __name__ == '__main__':
    ...
    username = '<USERNAME>'
    password = '<PASSWORD>'
    search_query = 'search index=os OR index=java splunk_server=wfc-* host=wfc-v30-pre-00* source=/var/log/jboss/client-profile-events.log earliest=1428351821 \"Home.Home.profiler beaconing data\" | stats perc95(val) as perc95'
